<?php
include_once 'header-footer/header.php' // Include header of page
?>

<head>
  <title>Εργοδότης | Υπουργείο Εργασίας & Κοινωνικών Υποθέσεων</title>
</head>



<div class="bgded" style="background:white;">
  <div id="breadcrumb" class="hoc clear">
    <h6 class="heading font-x3">Εργοδότης</h6>
    <ul>
      <li><a style="color: #1c7aa8;" class="hyperlink" href="../index.php">ΑΡΧΙΚΗ</a></li>
      <li><span>&#47;</span> </li>
      <li>
        <p style="color: #363636;">ΕΡΓΟΔΟΤΗΣ</p>
      </li>
    </ul>
  </div>
</div>


<div class="wrapper row3">
  <main class="hoc mycontainer clear">
    <ul class="list-covid-info">
      <li>
        <p style="font-size: 15px;">Ενημερώσου για <a class="hyperlink" href="../covid/prevention.php">τα τελευταία μέτρα</a> σχετικά με την πανδημία και τις επιχειρήσεις.</p>
      </li>
    </ul>
    <ul class="nospace group services mylist">
      <li class="one_third first list-layout">
        <h6 class="heading clear-text-transform top-margin-13"><a class="font-size-heading" href="employee-remote/employee-remote.php">
            Εξ αποστάσεως εργασία</a></h6>
        <p class="font-size-15">
          Δήλωσε έναν εργαζόμενο για εξ αποστάσεως εργασία ή κάνε άρση της συγκεκριμένης δήλωσης.
        </p>
      </li>
      <li class="one_third first list-layout">
        <h6 class="heading clear-text-transform top-margin-13"><a class="font-size-heading" href="employee-suspension/employee-suspension.php">
            Αναστολή σύμβασης εργασίας</a></h6>
        <p class="font-size-15">
          Δήλωσε την αναστολή σύμβασης εργασίας για έναν εργαζόμενο ή κάνε άρση της εν λόγω δήλωσης.
        </p>
      </li>
    </ul>
    <div class="clear btmspace-50"></div>
  </main>
</div>

<?php
include_once 'header-footer/footer.php' // Include footer of page
?>